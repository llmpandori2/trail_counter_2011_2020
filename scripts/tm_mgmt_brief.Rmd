...
visit_table <- visit_est %>%
  ...
  # rename some values
  rename(`Entrance station visitors` = `Entrance station`, `Coastal area visitors` = `Coastal area`)
...
formattable(visit_table,
            ...
            `Percent total visitors to coastal area` = color_bar
            (cal_palette('tidepool')[1])))

remove(visit_table, visit_est2)
```
...
# setup plot
ggplot(data = filter(weekday, lot == 'Lot 1' & events >= 10),
...
ggplot(data = filter(hr_tide, lot == 'Lot 1'),
